...
from model import PFM, Appropriateness_Discriminator, Frame_Discrminator
from utils import AverageMeter, accuracy, binary_accuracy
from render import Render
from dataset import get_dataloader
from model.losses import VAELoss, GenLoss, DisLoss, div_loss_multi, temporal_loss, GenLoss_Const
...
def parse_arg():
    ...
    parser.add_argument('--person-branch', default="", type=str, help="checkpoint path of the person-specific branch")
    parser.add_argument('--resume-d-f', default="", type=str, help="checkpoint path of the frame discriminator of general branch")
    parser.add_argument('--resume-d-a', default="", type=str, help="checkpoint path of the appropriateness discriminator of general branch")
    parser.add_argument('-b', '--batch-size', default=1, type=int, metavar='N', help='mini-batch size (default: 4)')
    parser.add_argument('-lr-g', '--learning-rate-g', default=0.0001, type=float, metavar='LRG',
    ...
    parser.add_argument('--tem-p', default=0.002, type=float, help="hyperparameter for div-loss")
    parser.add_argument('--p-weight', default=0.00001, type=float, help="hyperparameter for the importance of person-specific factor")
...
def train(args, gen, train_loader, optimizer, criterion, epoch, writer=None, d_f=None, d_a=None, render=None):
    ...
    for batch_idx, (speaker_video_clip, speaker_audio_clip, speaker_emotion, speaker_3dmm, _, listener_personal_clip, _, listener_emotion, inappro_listener_emotion, listener_3dmm, inappro_listener_3dmm, listener_references, num_appropriate, num_inappropriate) in enumerate(tqdm(train_loader)):
        if torch.cuda.is_available():
            speaker_video_clip, speaker_audio_clip, listener_personal_clip,  listener_emotion, inappro_listener_emotion, listener_3dmm, listener_references, num_appropriate, num_inappropriate = \
                speaker_video_clip.cuda(), speaker_audio_clip.cuda(), listener_personal_clip.cuda(), listener_emotion.cuda(), inappro_listener_emotion.cuda(), listener_3dmm.cuda(), listener_references.cuda(),  num_appropriate.cuda(), num_inappropriate.cuda()
        ...
        # num_samples = num_appropriate

        if args.model == 'baseline':
            listener_3dmm_out, listener_emotion_out, _ = gen(speaker_video_clip, speaker_audio_clip, listener_personal_clip, num_samples)

            gen_loss, rec_loss, kld_loss = criterion(listener_emotion, listener_3dmm, listener_emotion_out, listener_3dmm_out,
                                                None, num_appropriate, num_samples)
            ...
            listener_3dmm_out, listener_emotion_out, person_specific_factor = gen(speaker_video_clip, speaker_audio_clip, listener_personal_clip, num_samples)
            ...
            if d_a != None:
                ...
                valid_a = d_a(speaker_emotion=speaker_emotion, listener_emotion=listener_emotion_out, speaker_3dmm=speaker_3dmm, listener_3dmm=listener_3dmm_out, repeat_interleave=num_samples, person_specific_factor=person_specific_factor, p_weight=args.p_weight)
            else:
                label_a = None
                valid_a = None

            gen_loss, g_a_loss, kld_loss, const_loss, g_f_loss = criterion[0](valid_f, label_f, valid_a, label_a, None, listener_emotion, listener_emotion_out, num_appropriate, num_samples)
            ...
            if d_a != None:
                ...
                pred_real = d_a(speaker_emotion=speaker_emotion, listener_emotion=listener_emotion, speaker_3dmm=speaker_3dmm, listener_3dmm=listener_3dmm, repeat_interleave=num_appropriate, person_specific_factor=person_specific_factor.detach(), p_weight=args.p_weight)
                d_a_real_loss = criterion[2](pred_real, real)

                pred_fake = d_a(speaker_emotion=speaker_emotion, listener_emotion=listener_emotion_out.detach(), speaker_3dmm=speaker_3dmm, listener_3dmm=listener_3dmm_out.detach(), repeat_interleave=num_samples, person_specific_factor=person_specific_factor.detach(), p_weight=args.p_weight)
                d_a_fake_loss = criterion[2](pred_fake, fake)

                if num_inappropriate.sum() != 0:
                    inap = torch.cuda.FloatTensor(num_inappropriate.sum(), 1).fill_(0.0)
                    pred_inap = d_a(speaker_emotion=speaker_emotion, listener_emotion=inappro_listener_emotion, speaker_3dmm=speaker_3dmm, listener_3dmm=inappro_listener_3dmm, repeat_interleave=num_inappropriate, person_specific_factor=person_specific_factor.detach(), p_weight=args.p_weight)
                    d_a_inap_loss = criterion[2](pred_inap, inap)
                else:
                    d_a_inap_loss = torch.tensor(0.0).to(listener_emotion.get_device())

                target = torch.cat([real, fake, inap]) if num_inappropriate.sum() != 0 else torch.cat([real, fake])
                output = torch.cat([pred_real, pred_fake, pred_inap]) if num_inappropriate.sum() != 0 else torch.cat([pred_real, pred_fake])
...
def val(args, gen, val_loader, render, epoch):
    ...
    for batch_idx, (speaker_video_clip, speaker_audio_clip, speaker_emotion, speaker_3dmm, _, listener_personal_clip, _, listener_emotion, inappro_listener_emotion, listener_3dmm, inappro_listener_3dmm, listener_references, num_appropriate, num_inappropriate) in enumerate(tqdm(val_loader)):
        if torch.cuda.is_available():
            speaker_video_clip, speaker_audio_clip, listener_personal_clip, listener_emotion, inappro_listener_emotion, listener_3dmm, listener_references, num_appropriate, num_inappropriate = \
                speaker_video_clip.cuda(), speaker_audio_clip.cuda(), listener_personal_clip.cuda(), listener_emotion.cuda(), inappro_listener_emotion.cuda(), listener_3dmm.cuda(), listener_references.cuda(), num_appropriate.cuda(), num_inappropriate.cuda()
        ...
        with torch.no_grad():
            ...
            listener_3dmm_out, _, _ = gen(speaker_video_clip, speaker_audio_clip, listener_personal_clip, num_samples)
...
def main(args):
    ...
    train_loader = get_dataloader(args, "train", "person_specific", load_audio=True, load_video_s=True, load_emotion_s=True, load_clip_p=True,  load_emotion_l=True,  load_3dmm_l=True, load_3dmm_s=True, load_ref=True)
    val_loader = get_dataloader(args, "val", "person_specific", load_audio=True, load_video_s=True, load_emotion_s=True, load_clip_p=True,  load_emotion_l=True, load_3dmm_l=True, load_3dmm_s=True, load_ref=True)

    gen = PFM(img_size = args.img_size, audio_dim = args.audio_dim, output_3dmm_dim = args._3dmm_dim, output_emotion_dim = args.emotion_dim, feature_dim = args.feature_dim, seq_len = args.clip_length, general_branch_path=args.general_branch, integration=args.integration, device = args.device, k=args.k_parameter, beta=args.beta)
    ...
    if args.model == 'baseline':
        ...
        d_f = Frame_Discrminator(feature_dim=58)
        d_a = Appropriateness_Discriminator()
        criterion = [GenLoss(args.kl_p), DisLoss('bce'), DisLoss('mse')]
        ...
        if args.resume_d_f != '':
            checkpoint_path = args.resume_d_f
            print("Load the discriminator from {}".format(checkpoint_path))
            checkpoints = torch.load(checkpoint_path, map_location=torch.device('cpu'))
            state_dict = checkpoints['state_dict']
            d_f.load_state_dict(state_dict)
            
            # Only use for resuming from interruption
            # optimizer[1].load_state_dict(checkpoints['optimizer'])
        ...
        if args.resume_d_a != '':
            checkpoint_path = args.resume_d_a
            print("Load the discriminator from {}".format(checkpoint_path))
            checkpoints = torch.load(checkpoint_path, map_location=torch.device('cpu'))
            state_dict = checkpoints['state_dict']
            ...
            # Only use for resuming from interruption
            # optimizer[2].load_state_dict(checkpoints['optimizer'])

            # for state in optimizer[2].state.values():
            #     for k, v in state.items():
            #         if isinstance(v, torch.Tensor):
            #             state[k] = v.to('cuda')
    
    # print('gen parameters:', sum([param.nelement() for param in gen.parameters()]))

    if args.resume != '':   # Load the generator/baseline model
        ...
        # Only use for resuming from interruption
        # optimizer[0].load_state_dict(checkpoints['optimizer'])
